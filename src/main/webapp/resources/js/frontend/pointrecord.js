$(function() {
	...
	// 1 按照查询条件获取奖品兑换记录列表,并生成对应的HTML元素添加到页面中
	function addItems(pageSize, pageNum) {
		// 1 生成新条目的HTML
		var url = listUrl + '?pageIndex=' + pageNum+ '&pageSize=' + pageSize
		 + '&awardName=' + awardName;
		loading = true;
		$.getJSON(url, function(data) {
			if (data.success) {
				// 没有信息则给出提示
				if(data.userAwardMapList.length==0){
